...
export default function NewsGrid({ items = [], category }) {
  ...
  useEffect(() => {
    if (items && items.length) return; // si nos pasan items, usamos esos
    const url = category ? `${BASE_URL}/news?category=${category}` : `${BASE_URL}/news`;
    fetch(url).then(r => r.json()).then(setNews).catch(() => setNews([]));
  }, [items, category]);
        ...
        <div key={n.id} className="card">
          {n.image_url && (
            <img 
              ...
              alt={n.title}
              style={{ borderRadius: "8px", marginBottom: 12, width: "100%", height: "200px", objectFit: "cover" }} 
            />
          )}
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h3 style={{ margin: 0 }}>{n.title}</h3>
            {n.category && (
              <span style={{ background: 'var(--color-accent)', color: 'white', padding: '2px 8px', borderRadius: 12, fontSize: '0.8em' }}>
                {n.category}
              </span>
            )}
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontSize: '0.9em', color: 'var(--color-muted)' }}>
              {n.created_at ? new Date(n.created_at).toLocaleDateString('es-ES') : ''}
            </span>
