...
export default function AdminPortal() {
  ...
  async function approveApp(id) {
    const r = await apiPost(`/admin/applications/${id}/approve`, { note: "Ok" });
      ...
      <div className="container">
        ...
        <div className="cards">
          {apps.map(a => (
            <div key={a.id} className="card">
              <h4>{a.name} ({a.email})</h4>
              <p>Estado: {a.status}</p>
              <button className="btn btn-outline" onClick={() => openAppDetails(a)}>
                Ver completo
              </button>
            </div>
          ))}
        ...
        {newsByStatus.pending.length > 0 && (
          <div style={{ marginBottom: 32 }}>
            <h4 style={{ color: 'var(--color-accent)', marginBottom: 16 }}>Pendientes de revisión</h4>
            <div className="cards">
              {newsByStatus.pending.map(n => (
                <div key={n.id} className="card">
                  ...
                  />
                  <a href={`/admin/news/${n.id}/view`} className="btn btn-outline" target="_blank">
                    Ver completa
                  </a>
                </div>
              ))}
        ...
        {newsByStatus.published.length > 0 && (
          <div style={{ marginBottom: 32 }}>
            <h4 style={{ color: 'var(--color-secondary)', marginBottom: 16 }}>Publicadas</h4>
            <div className="cards">
                ...
                <div key={n.id} className="card">
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
                    ...
                    <div style={{ marginLeft: 16, textAlign: 'center' }}>
                      <div style={{ fontSize: '12px', color: 'var(--color-muted)', marginBottom: 8 }}>
                        Orden: {n.order_index}
                      </div>
                  ...
                  />
                  <a href={`/noticias/${n.id}`} className="btn btn-outline" target="_blank">
                    Ver completa
                  </a>
                </div>
              ))}
        ...
        {newsByStatus.rejected.length > 0 && (
          <div>
            <h4 style={{ color: 'var(--color-muted)', marginBottom: 16 }}>Rechazadas</h4>
            <div className="cards">
          ...
          <div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 24 }}>
              <div>
                ...
                <p><strong>Nombre:</strong> {selectedApp.name}</p>
                <p><strong>Email:</strong> {selectedApp.email}</p>
                <p><strong>Teléfono:</strong> {selectedApp.phone || 'No especificado'}</p>
                <p><strong>Estado:</strong> {selectedApp.status}</p>
                <p><strong>Fecha de solicitud:</strong> {new Date(selectedApp.created_at).toLocaleDateString('es-ES')}</p>
              </div>
              <div>
                <h4>Motivación</h4>
                <div style={{ background: '#f5f5f5', padding: 12, borderRadius: 8, minHeight: 100 }}>
            ...
            {/* Botones de acción */}
            <div style={{ display: 'flex', gap: 16, justifyContent: 'flex-end', borderTop: '1px solid #eee', paddingTop: 16 }}>
              <button className="btn btn-outline" onClick={() => setSelectedApp(null)}>
                Cancelar
              </button>
              <button className="btn btn-outline" onClick={() => rejectApp(selectedApp.id)}>
                Rechazar
              </button>
              <button className="btn btn-primary" onClick={() => approveApp(selectedApp.id)}>
                Aprobar
              </button>
            </div>
          </div>
