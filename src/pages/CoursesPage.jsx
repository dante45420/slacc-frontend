...
export default function CoursesPage() {
  ...
  function getPriceForUser(course) {
    ...
    if (user.membership_type === "joven") {
      return course.price_joven > 0 ? course.price_joven : course.price_member;
    } else if (user.membership_type === "gratuito") {
      return course.price_gratuito > 0 ? course.price_gratuito : 0;
    } else {
      return course.price_member;
    }
  }
      ...
      <section className="section">
        <div className="container">
          <div style={{ textAlign: 'center', padding: '48px 0' }}>
            <div style={{ fontSize: '1.2em', color: 'var(--color-muted)' }}>Cargando cursos...</div>
          </div>
        </div>
      </section>
    );
      ...
      <div className="container">
        <div style={{ textAlign: 'center', marginBottom: 48 }}>
          ...
          <p style={{ fontSize: '1.2em', color: 'var(--color-muted)', maxWidth: 600, margin: '0 auto' }}>
            Amplía tus conocimientos con nuestros cursos especializados en cirugía de cadera
          </p>
        </div>

        {courses.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '48px 0' }}>
            <div style={{ fontSize: '1.2em', color: 'var(--color-muted)', marginBottom: 16 }}>
                ...
                <div key={course.id} className="card" style={{ position: 'relative' }}>
                  ...
                  <div style={{ marginBottom: 16 }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                      ...
                      <span>{course.max_students ? `${course.max_students} estudiantes` : 'Sin límite'}</span>
                    </div>
                  </div>
                  ...
                  <div style={{ 
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div>
                        ...
                        {discount > 0 && (
                          <div style={{ fontSize: '0.9em', color: 'var(--color-secondary)' }}>
                      ...
                      <div style={{ textAlign: 'right', fontSize: '0.9em', color: 'var(--color-muted)' }}>
                        {user ? (
                          <div>
                            ...
                            <div style={{ textDecoration: 'line-through', color: 'var(--color-muted)' }}>
...
function EnrollmentModal({ course, user, onClose, onSuccess }) {
  ...
  const userPrice = user ? 
    (user.membership_type === "joven" ? (course.price_joven || course.price_member) :
     user.membership_type === "gratuito" ? (course.price_gratuito || 0) :
     course.price_member) : course.price_non_member;
  ...
  async function handleEnroll() {
    if (step === 1) {
      ...
      try {
        setLoading(true);
        const result = await apiPost(`/courses/${course.id}/enroll`, formData);
        setStep(3);
      } catch (err) {
  ...
  async function confirmPayment() {
    try {
      ...
      await apiPost(`/courses/${course.id}/enrollments/${course.enrollment_id}/confirm-payment`);
      onSuccess();
    } catch (err) {
      ...
      <div style={{
        ...
        {step === 1 && (
          <div>
            ...
            <p style={{ color: 'var(--color-muted)', marginBottom: 24 }}>{course.description}</p>
            ...
            <div style={{ 
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <span style={{ fontSize: '1.5em', fontWeight: 'bold', color: 'var(--color-primary)' }}>
                  ${userPrice}
                </span>
              </div>
              {user && (
                <div style={{ fontSize: '0.9em', color: 'var(--color-secondary)', marginTop: 4 }}>
        ...
        {step === 3 && (
          <div style={{ textAlign: 'center' }}>
            ...
            <p style={{ color: 'var(--color-muted)', marginBottom: 24 }}>
              Te has inscrito correctamente al curso. Recibirás un email de confirmación.
            </p>
          </div>
        )}
        ...
        <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end' }}>
          ...
          >
            {loading ? 'Procesando...' : 
             ...
             step === 2 ? 'Confirmar Pago' : 'Finalizar'}
          </button>
        </div>
      </div>
    </div>
